...
- **Approach (three stages, per the review):**
  ...
  - **Stage 1 — mandatory gates (certification, per construct, *executable* confidence rule):**
    each mandatory construct has a threshold on the **Wilson score interval** (95%) of its
    success/error rate, evaluated on the **certification partition (P3)**.
    ⚠️ **Feasibility constraint (a PR review caught my v1 here — see Provenance):** the threshold and
    `n` must be JOINTLY feasible. A Wilson 95% bound of a *perfect* score is far from 1 at small `n`
    (10/10 → LB ≈ 0.72; 0/10 → UB ≈ 0.28), so the original "LB ≥ 0.80 / UB ≤ 0.10 at `n=10`" rejected
    even a flawless config — **impossible, not strict.** RULE: pick `n` so a NEAR-perfect config
    (≤1–2 errors) clears the bar with margin. Corrected example: the load-bearing fabrication gates at
    **`n ≈ 20–30`** — `fabrication-recall LB ≥ 0.75` (20/20 → LB ≈ 0.84, tolerates ~1 miss) and
    `clean-false-DQ UB ≤ 0.25` (0–1 false-DQ passes); softer constructs (A/B-reversal,
    improvement-recognition) at **`n ≥ 15`**, bounds set the same way. **Any** mandatory construct
    below its bound ⇒ config fails (no averaging). **Stopping rule:** successive halving — screen
    candidates on the cheap dev partition, keep the top half, spend the larger certification items
    only on survivors. (A handful of observations never certifies: 5/5 → Wilson-LB ≈ 0.57.)
  - **Budget reconciliation:** feasible Wilson gates push `n` toward **20–30** for the critical
    fabrication families, so a credible maximal tier lands at the **upper end (~250–400 judgments)**,
    not 100–250. Absorbed by P3 (the bank is built/adjudicated **once** and reused — certification
    only *spends* judgments, not generation) plus successive halving (only the 1–2 surviving configs
    reach the larger certification partition). Lever: cap the live run to **K ≤ 4 candidate configs**
    and **C = the mandatory constructs only** (quality-construct dashboards are diagnostic, not
    gating). State the chosen K, per-family `n`, and which constructs gate, per run.
...
| **"Confidence-bounded" is unexecutable** (no thresholds / n / interval / stop) | Wilson-LB ≥ per-construct thresholds with **jointly-feasible `n`** (≈20–30 for fabrication gates, so a near-perfect config clears — `n=10`+0.80 is impossible, per PR review), successive-halving stop; budget reconciled by amortizing P3 + capping K ≤ 4 |
| **Impossible Wilson gates** (threshold a perfect score can't clear at small `n`) | feasibility rule: pick `n` so `≤1–2 errors` clears with margin; corrected example LB ≥ 0.75 at `n≈20–30`, not LB ≥ 0.80 at `n=10` |
...
**A PR-review pass on PR #3 (chatgpt-codex-connector bot, 2026-06-20)** caught that the U12 Stage-1
Wilson-gate numbers were **statistically impossible**: a 95% Wilson LB of a perfect 10/10 is ≈0.72
(< the required 0.80) and a 0/10 false-DQ upper bound is ≈0.28 (> the required 0.10), so an
implementation would reject *every* config despite zero observed errors. Corrected: added the
**feasibility constraint** (threshold × `n` must let a near-perfect config clear with margin) and
re-specified the load-bearing fabrication gates at `n ≈ 20–30` with `LB ≥ 0.75` / `UB ≤ 0.25`,
updating the budget to the ~250–400 upper end (absorbed by P3 amortization + halving). (This was the
**only real PR-#3 review finding**; a parallel local/external code-review tool produced separate
U19-code findings that were addressed directly in commits `23d9ebc`/`377d96f`.)
