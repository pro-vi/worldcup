...
function standings(group, gi, results) {
  ...
  // Sort by a CONSISTENT comparator (points, then seeding rating). Head-to-head must NOT live
  // inside the comparator: under a 3-way beat cycle (A>B>C>A on equal points) it is non-transitive,
  // which hands Array.sort an inconsistent ordering and makes 3rd place — a qualification slot in
  // the 48 format — arbitrary. Head-to-head then breaks CLEAN 2-way ties only; cycles keep rating order.
...
function r32Pairs(rankedQualifiers) {
  ...
  // same-group repair as a bounded fixpoint: swapping second teams between neighboring pairs can
  // CREATE a new conflict at the next pair (and a single pass never re-checks it, or the last pair
  // at all) — so re-scan, wrapping, until clean or the pass bound trips. A residual conflict after
  // that many passes means the draw itself is degenerate; play it as-is.
  for (let pass = 0, dirty = true; dirty && pass < pairs.length; pass++) {
    dirty = false
