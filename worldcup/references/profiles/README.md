...
3. **A profile file you wrote earlier in `references/profiles/<your-domain>.md`** → reuse it.
   (None ship by default — see Writing one below.)
...
## Writing one
No example profile ships here — taste is yours to bring. Copy the four-part skeleton below
into `references/profiles/<your-domain>.md`, or hand the skill your own voice skill (path 1)
and let it distill the rules. Keep style tics as lens penalties, not gate kills, unless you
genuinely never want the thing. Each part maps onto a named hole in `workflow-template.js`:

```markdown
# <your-domain> profile

## Taste spec        <!-- → the TASTE line of CRITERIA_BASE; the judges read it verbatim -->
- <the positive qualities a strong entry has — deep traits, not a checklist>

## Gate              <!-- → hard-DQ subtypes: add to HARD_DQ_CATEGORIES, map each to a family in DQ_FAMILY -->
- <e.g. prose: FALSE_AUTHORIAL_EXPERIENCE → family 'fabrication'; code: FAKE_TEST_PASS → 'fabrication'>

## Lenses            <!-- → entries in LENSES (name → one-axis mandate); seat them via panelFor -->
- <lens name>: <its ruthless one-axis mandate>

## House bans        <!-- OPTIONAL → the BANS object: { emDash, vocab: [...], softPatterns: [{ label, re, tail? }] } -->
- <hard bans only for things you never want; style tics belong in the lenses>
```
